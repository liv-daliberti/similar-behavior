...
for ALPHA in "${ALPHAS[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        # Construct run name: sac_finger-spin-alpha-Y-seed-X
        RUN_NAME="sac_finger-spin-alpha-${ALPHA}-seed-${SEED}"
        ...
        python cleanrl/cleanrl/sac_continuous_action_2.py \
            ...
            --alpha ${ALPHA} \
            --no-autotune \
            --env_id "dm_control/finger-spin-v0" \
            --exp_name dmcontrol-finger-spin-v0 \
            --wandb_project_name dmcontrol_finger \
            --run_name ${RUN_NAME} \
            --track \
            --torch_deterministic \
            --cuda \
