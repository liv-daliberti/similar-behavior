...
This version only performs the Q-difference experiment (mean delta Q per episode)
and writes to a new output file without overwriting previous results.

Changes:
 - Computes **mean** Q-value differences per episode (rather than max)
 - Skips KL divergence, reward, and Jacobian comparisons
 - Writes to a new file: "{env}-pairwise_q_mean_diff.csv"
"""

# === Imports and utility functions remain unchanged ===
import os
import glob
import yaml
import torch
...
def evaluate_q_output_diff_episode(env, actor_1, qf1_i, qf2_i, qf1_j, qf2_j, device, seed=0):
    ...
    return total_q_diff / max(1, steps)
...
def evaluate_q_output_diff_episode_meow(env, actor_1, actor_2, device, seed=0):
    ...
    while not done:
        ...
        with torch.no_grad():
            ...
            current_diff = (q1 - q2).abs().mean().item()
            total_q_diff += current_diff
            steps += 1
...
HEADER = (
    ...
    "episode_idx,episode_q_meandiff\n"
)
...
def main(cfg: DictConfig):
    ...
    for env in cfg.envs:
        ...
        output_file = os.path.join(results_dir, f"{env}-pairwise_q_mean_diff.csv")
                ...
                for episode_idx in range(cfg.n_eval_episodes):
                    ...
                    with open(output_file, "a", newline="") as fh:
                        fh.write(
                            ...
                            f"{episode_idx},{ep_qdiff}\n"
                        )
                    existing_keys.add(row_key)

        logger.info(f"Q mean difference results for {env} written/updated at {output_file}")


if __name__ == "__main__" and os.path.basename(__file__) == "q-diff.py":
    main()
